...
Label2D::Label2D(std::string _text, glm::vec2 _position, std::string _font, GLfloat _scale) {
    text = _text;
    scale = _scale;
    position = _position;
    scale = _scale;
...
int Label2D::draw()
{
    ...
    if (!_isInit) {
        assert("Label2D has not been initialized!");
    }
